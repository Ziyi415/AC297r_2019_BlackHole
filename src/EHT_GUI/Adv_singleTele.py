...
class Window(QWidget):  
    ...
    def setTable(self, df):
        ...
        self.columnList = ['Name', 'Start Time', 'End Time', 'Weight','Availability']
        for i in range(5):
        ...
        for r in range(self.table.rowCount()):
            for c in range(5):
                if c == 0:
                    ...
                    self.new_avail.append(int(self.table.item(r, c).text()))
        return pd.DataFrame({'Name':self.new_name, 'Start Time':self.new_start, 'End Time': self.new_end, 'Weight': self.new_weight, 'Availability': self.new_avail})
